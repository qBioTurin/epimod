	...
	for (i in dim(containers)[1]:1)
	{
		...
		else
		{
			...
			if (grepl("generation",containers[i,1],fixed=TRUE)==1)
				command=c(paste("FROM", containers[i,1]),"RUN id -u","RUN sudo usermod -u 2005 docker", paste("RUN sudo /usr/sbin/adduser -u", userid, username))
			else
				command=c(paste("FROM", containers[i,1]),"RUN id -u","RUN sudo usermod -u 2005 docker",paste("RUN /usr/sbin/adduser -u", userid, username), "WORKDIR /home" )
