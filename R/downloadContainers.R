	...
	for (i in dim(containers)[1]:1)
	{
		...
		else
		{
			...
			if (grepl("generation",containers[i,1],fixed=TRUE)==1)
				command=c(paste("FROM", containers[i,1]),"RUN  groupmod -g 2005 docker &&  usermod -u 2005 docker ", paste("RUN  /usr/sbin/adduser --allow-bad-names -u", userid, username), "  WORKDIR /home")
			else
