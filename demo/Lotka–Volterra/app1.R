...
ui <- fluidPage( fluidRow( column(3,
                 ...
                 fluidRow(column(2, offset=1, verbatimTextOutput("text")),
                          column(2, offset=1, verbatimTextOutput("ls")),
                          column(2, offset=1, verbatimTextOutput("folders")))
                 )
...
server <- function(input, output, session) {
    ...
    output$plot_phase <- renderPlot({
        ...
        if(input$n_traces > 0 && length(rnk) > 0)
        {
            ...
            ListTraces<-lapply((1:min(input$n_traces,length(rnk$id))),
                               function(x){
                                   if(file.exists(file.path(input$dir,rnk$file_name[x], fsep = .Platform$file.sep)))
                                   {
    ...
    output$ls <- renderPrint({
        rv$ls
    })
    output$folders <- renderPrint({
        rv$folders
    })
}
